;(function () {
  ...
  var MANIFEST = {
    ...
    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2 2 7l10 5 10-5-10-5Z"/><path d="m2 17 10 5 10-5"/><path d="m2 12 10 5 10-5"/></svg>',
  }
  ...
  // --- Функция удалённого логирования ---
  function remoteLog() {
  ...
  function addLang() {
    Lampa.Lang.add({
      ...
      jellyfin_user_auto: {
        en: "First user (auto)",
        ru: "Первый пользователь (авто)",
      },
      jellyfin_log_url: {
        en: "Log URL (optional)",
        ru: "URL для логов (опционально)",
      },
    })
  }
  ...
  function jfHttp(path, opts) {
    ...
    var key = apiKey()
    if (!base || !key)
      return Promise.reject(new Error("Jellyfin URL or API key is empty"))
    ...
    var timeout =
      typeof opts.timeout === "number" ? opts.timeout : HTTP_TIMEOUT_MS
    var dataType = opts.dataType || "json"
    var method = (opts.method || "GET").toUpperCase()
    var postData =
      method === "POST" && opts.jsonBody === undefined ? opts.data : undefined
    var net = network()
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (useJsonAjax) {
        $.ajax({
          ...
          contentType:
            opts.jsonBody !== undefined ? "application/json" : undefined,
          data:
      ...
      if (!net) {
        Lampa.Network.silent(url, ok, fail, postData, {
          timeout: timeout,
          dataType: dataType,
        })
        return
      ...
      net.timeout(timeout)
      net.silent(url, ok, fail, postData, {
        timeout: timeout,
        dataType: dataType,
      })
    })
  ...
  function tmdbJson(url) {
    ...
    var inner = new Promise(function (resolve, reject) {
      ...
      net.silent(url, resolve, reject, null, {
        timeout: TMDB_TIMEOUT_MS,
        dataType: "json",
      })
    })
  ...
  function fetchUsers() {
    return jfHttp("/Users").then(function (users) {
      if (!Array.isArray(users) || !users.length)
        throw new Error("No Jellyfin users")
      return users
    })
  }
  ...
  function syncUserInfoField() {
    var $descr = $(
      '[data-name="' + STORAGE_PREFIX + 'UserInfo"] .settings-param__descr'
    )
  ...
  function getDeviceId() {
    ...
    id =
      "lampa-" +
      (Lampa.Utils && Lampa.Utils.uid ? Lampa.Utils.uid() : String(Date.now()))
  ...
  function activePlayerId() {
    try {
      return String(
        Lampa.Storage.field("player") ||
          Lampa.Storage.get("player", "inner") ||
          "inner"
      )
        .trim()
        .toLowerCase()
  ...
  function usesLampaNativePlayer() {
    ...
    if (
      typeof Platform.desktop === "function" &&
      Platform.desktop() &&
      player === "other"
    ) {
      return false
  ...
  function fetchPlaybackInfo(itemId, userId, opts) {
    ...
    remoteLog("[Jellyfin] fetchPlaybackInfo called", {
      itemId,
      userId,
      startTicks,
    })
  ...
  function buildPlayObject(row, userId, startTicks) {
    var itemId = row.id
    remoteLog("[Jellyfin] buildPlayObject start", {
      itemId,
      userId,
      startTicks,
    })
    ...
    return fetchPlaybackInfo(itemId, userId, { startTicks: startTicks })
      .then(function (info) {
        ...
        if (!transcodingUrl) {
          remoteLog(
            "[Jellyfin] No TranscodingUrl, requesting with MediaSourceId and enableTranscoding=true"
          )
      ...
      .then(function (result) {
        ...
        var defAudio = streams.find(function (s) {
          return s.Type === "Audio" && s.IsDefault === true
        })
        var defSub = streams.find(function (s) {
          return s.Type === "Subtitle" && s.IsDefault === true
        })
        ...
        var playObj = {
          ...
          subtitles: [
            {
              url: apiBase() + currentSubtitleDeliveryUrl,
        ...
        remoteLog("[Jellyfin] final playObj:", playObj)
        return playObj
      })
  }

  // --- Функция обновления плеера при смене аудио/субтитров ---
  function updatePlayerWithNewStreams(
    itemId,
    userId,
    audioIdx,
    subIdx,
  ...
  function setupTracksForJellyfin() {
    Lampa.Player.listener.follow("ready", function (data) {
      ...
      var audioStreams = currentMediaStreams.filter(function (s) {
        return s.Type === "Audio"
      })
      var subStreams = currentMediaStreams.filter(function (s) {
        ...
        Object.defineProperty(track, "enabled", {
          set: function (v) {
            if (v) {
              ...
              var startTicks =
                Lampa.Player.playdata() && Lampa.Player.playdata().timeline
                  ? Lampa.Player.playdata().timeline.time * 10000000
                  : 0
              updatePlayerWithNewStreams(
                itemId,
                userId,
                stream.Index,
      ...
      var subs = subStreams.map(function (stream) {
        ...
        Object.defineProperty(sub, "mode", {
          ...
          get: function () {
            return sub.selected ? "showing" : "disabled"
          },
        })
        return sub
      })
  ...
  function episodeTitle(item, seriesTitle) {
    ...
    if (
      epName &&
      !/^s\d+\s*e\d+/i.test(epName) &&
      !RELEASE_FOLDER_RE.test(epName)
    ) {
    ...
    return {
      ...
      rating:
        item.CommunityRating && Number(item.CommunityRating) > 0
          ...
          : "",
      resumeSec: item.UserData
        ? ticksToSeconds(item.UserData.PlaybackPositionTicks)
        : 0,
      playedPct: item.UserData
        ? Number(item.UserData.PlayedPercentage) || 0
        : 0,
  ...
  function dedupeRows(rows) {
    ...
    rows.forEach(function (row) {
      if (row.tmdb) {
        ...
        if (!best[key] || itemScore(row.raw) > itemScore(best[key].raw))
          best[key] = row
      } else {
        loose.push(row)
      }
  ...
  function fetchItems(category, startIndex) {
    return resolveUserId().then(function (userId) {
      ...
      return jfHttp(listPath(category, userId, startIndex)).then(function (
        data
      ) {
  ...
  function bindJellyfinCard($card, row, ctx) {
    ...
    $card.on("jf:update", function (_e, updated) {
      ...
      $card
        .find(".card__title")
        .text(ctx && ctx.compact ? hubCardTitle(updated) : updated.title)
  ...
  function makeJellyfinCard(row, ctx) {
    ...
    if (ctx.cardsById)
      ctx.cardsById[String(row.id)] = { $card: $card, row: row }
    return $card
  }
  ...
  function detachHubRowListener(hubCtx) {
    if (hubCtx.onRowUpdated)
      Lampa.Listener.remove("jellyfin:row-updated", hubCtx.onRowUpdated)
  }
      ...
      fetchHubData()
        .then(function (data) {
          ...
          if (Lampa.Layer && Lampa.Layer.visible)
            Lampa.Layer.visible(scroll.render()[0])
          if (lines.length) {
            try {
  ...
  function HubLineFallback(data, hubCtx) {
    ...
    content.addClass(
      "items-line--type-" + (data._jf_stats ? "default" : "cards")
    )
    ...
    this.create = function () {
      ...
      ;(data.results || []).forEach(function (element) {
        ...
        if (element._jf_stat) {
          ...
          $render
            .find(".register__counter")
            .text(String(stat.value == null ? 0 : stat.value))
        ...
        $render.on("hover:focus", function (e) {
          ...
          if (focusBg && focusBg !== IMG_PLACEHOLDER)
            Lampa.Background.change(focusBg)
        })
  ...
  function refreshLibraryIndex(force) {
    if (
      !force &&
      libraryIndex.loadedAt &&
      Date.now() - libraryIndex.loadedAt < LIBRARY_INDEX_TTL_MS
    ) {
        ...
        if (row.type === "Series") {
          fetchEpisodes(row.id)
            .then(function (eps) {
              if (!eps.length) {
                Lampa.Bell.push({
                  text: Lampa.Lang.translate("jellyfin_empty"),
                })
                return
              }
      ...
      if (transcodingEnabled()) {
        buildPlayObject(row, userId, startTicks)
          .then(function (playObj) {
            remoteLog(
              "[Jellyfin] About to call Lampa.Player.play with",
              playObj
            )
        ...
        remoteLog("[Jellyfin] External player playObj", playObj)
        Lampa.Player.play(playObj)
      }
    })
  }
  ...
  function streamUrl(itemId, opts) {
    ...
    if (opts.startTicks > 0)
      parts.push(
        "StartTimeTicks=" + encodeURIComponent(String(opts.startTicks))
      )
    if (opts.playSessionId)
      parts.push("PlaySessionId=" + encodeURIComponent(opts.playSessionId))
    if (opts.audioStreamIndex !== undefined && opts.audioStreamIndex !== null) {
      parts.push(
        "AudioStreamIndex=" + encodeURIComponent(opts.audioStreamIndex)
      )
    }
    if (
      opts.subtitleStreamIndex !== undefined &&
      opts.subtitleStreamIndex !== null
    ) {
      parts.push(
        "SubtitleStreamIndex=" + encodeURIComponent(opts.subtitleStreamIndex)
      )
    }
  ...
  function showItemMenu(row) {
    ...
    var items = [
      { title: Lampa.Lang.translate("jellyfin_play"), action: "play" },
    ]

    if (row.tmdb || row.type === "Episode" || row.type === "Series") {
      items.push({
        title: Lampa.Lang.translate("jellyfin_open_card"),
        action: "card",
      })
    }
    if (row.type === "Series") {
    ...
    items.push({
      title: Lampa.Lang.translate(
        row.watched ? "jellyfin_mark_unwatched" : "jellyfin_mark_watched"
      ),
  ...
  function injectCardChrome($card, row, opts) {
    ...
    var $chrome = $(
      '<div class="jellyfin-card-chrome" aria-hidden="true"></div>'
    )
    ...
    if (row.watched || row.playedPct >= 100) {
      $chrome.append(
        '<div class="jellyfin-badge jellyfin-badge-watched">✓</div>'
      )
    }
  ...
  function updateCardPoster($card, row) {
    ...
    if (src && src !== IMG_PLACEHOLDER)
      $card.find(".card__img").attr("src", src)
  }
  ...
  function PanelComponent(object) {
    ...
    var body = $(
      '<div class="category-full mapping--grid cols--6 jellyfin-grid"></div>'
    )
    ...
    function renderHead() {
      head.html(
        '<div class="jellyfin-head__title">' +
          $("<div>").text(headTitle()).html() +
          "</div>"
      )
    }
    ...
    function renderEmpty(opts) {
      ...
      $box.append(
        '<div class="jellyfin-state__title">' +
          $("<div>")
            .text(opts.title || Lampa.Lang.translate("jellyfin_empty"))
            .html() +
          "</div>"
      )
      $box.append(
        '<div class="jellyfin-state__descr">' +
          $("<div>")
            .text(opts.descr || Lampa.Lang.translate("jellyfin_empty_descr"))
            .html() +
          "</div>"
      )
      var $retry = $(
        '<div class="simple-button selector">' +
          Lampa.Lang.translate("jellyfin_retry") +
          "</div>"
      )
  ...
  function registerMenuButtons() {
    Lampa.Menu.addButton(
      MANIFEST.icon,
      Lampa.Lang.translate("jellyfin_title"),
      openHub
    )
  }
  ...
  function addSettings() {
    ...
    Lampa.SettingsApi.addParam({
      component: SETTINGS_COMPONENT,
      param: {
        name: STORAGE_PREFIX + "LogUrl",
        type: "input",
        default: "",
        values: "",
      },
      field: { name: Lampa.Lang.translate("jellyfin_log_url") },
      onChange: function () {
        Lampa.Settings.update()
      },
    })
    ...
    Lampa.SettingsApi.addParam({
      ...
      onChange: function () {
        resolveUserId()
          ...
          .catch(function () {
            Lampa.Bell.push({
              text: Lampa.Lang.translate("jellyfin_auth_fail"),
            })
          })
      },
    })
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        type: "trigger",
        default: true,
        name: STORAGE_PREFIX + "HideFolders",
      },
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        type: "trigger",
        default: true,
        name: STORAGE_PREFIX + "TmdbPosters",
      },
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        type: "trigger",
        default: true,
        name: STORAGE_PREFIX + "FullButton",
      },
